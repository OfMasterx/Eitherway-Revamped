...
export default defineConfig((config) => {
  return {
    server: {
      ...
      proxy: {
        '/api': {
          target: `https://localhost:${process.env.VITE_BACKEND_PORT || '3001'}`,
          changeOrigin: true,
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use((req, res, next) => {
        ...
        // Enable crossOriginIsolated for WebContainers
        // NOTE: This will cause Coinbase Wallet SDK to show a warning, but that's non-critical
        // WebContainer requires COOP: same-origin to enable SharedArrayBuffer
