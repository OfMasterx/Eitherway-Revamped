...
import { Agent, DatabaseAgent, ConfigLoader } from '../runtime/src/index.js';
import { getAllExecutors } from '../tools-impl/src/index.js';
import { createDatabaseClient, FilesRepository, SessionsRepository, PostgresFileStore, RateLimiter } from '../database/src/index.js';
...
// Resolve project root (go up from backend/src to integrated root)
...
const FEATURE_STREAMING_WS = process.env.FEATURE_STREAMING_WS !== 'false'; // Default: true
const FEATURE_FILE_OP_EVENTS = process.env.FEATURE_FILE_OP_EVENTS !== 'false'; // Default: true
...
fastify.register(async (fastify) => {
  fastify.get<{
    ...
    // Register WebSocket connection for file_op event broadcasting
    if (sessionId) {
      wsManager.register(sessionId, connection.socket);
    }
      ...
      if (data.type === 'prompt') {
        try {
          ...
          if (!USE_LOCAL_FS && dbConnected && db && sessionId) {
            ...
            // Phase 3: Send assistant_started if streaming enabled
            if (FEATURE_STREAMING_WS) {
              connection.socket.send(JSON.stringify({
          ...
          // Phase 3: Send response with streaming protocol
          if (FEATURE_STREAMING_WS) {
            // Send response as a single delta (simulates streaming)
            connection.socket.send(JSON.stringify({
            ...
            // Legacy: send as single response
            connection.socket.send(JSON.stringify({
              type: 'response',
              content: response
            }));
