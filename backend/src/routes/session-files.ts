...
export async function registerSessionFileRoutes(
  ...
  fastify.post<{
    ...
    try {
      // Phase 3: Emit file_op start event
      const FEATURE_FILE_OP_EVENTS = process.env.FEATURE_FILE_OP_EVENTS !== 'false';
      if (FEATURE_FILE_OP_EVENTS) {
      ...
      // Phase 3: Emit file_op complete event
      if (FEATURE_FILE_OP_EVENTS) {
        wsManager.broadcast(sessionId, {
          type: 'file_op',
          op: 'write',
      ...
      // Phase 3: Emit file_op error event
      const FEATURE_FILE_OP_EVENTS = process.env.FEATURE_FILE_OP_EVENTS !== 'false';
      if (FEATURE_FILE_OP_EVENTS) {
        wsManager.broadcast(sessionId, {
          type: 'file_op',
